...
export function SettlementsList({
  ...
  const { t } = useLanguage()
  return (
    <div className="min-w-0 rounded-xl border border-violet-500/20 bg-card shadow-sm overflow-hidden">
      <div className="overflow-x-auto">
      {/* Header */}
      <div className="flex items-center gap-x-4 px-5 py-2.5 text-[11px] font-bold text-violet-600 dark:text-violet-400 uppercase tracking-widest border-b border-violet-500/20 bg-linear-to-r from-violet-500/10 via-purple-500/5 to-transparent">
      ...
      {settlements.map((s) => {
        ...
        return (
          <div
            key={s.id}
            className="group flex items-center gap-x-4 px-5 py-3.5 border-b border-border/50 last:border-b-0 hover:bg-violet-500/5 transition-colors duration-150"
          >
            {/* Partners */}
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 min-w-0 text-sm font-medium text-foreground">
                <span className="truncate min-w-0 flex-1">{s.fromPartnerName}</span>
                <ArrowRight className="size-3.5 text-muted-foreground shrink-0" />
                <span className="truncate min-w-0 flex-1">{s.toPartnerName}</span>
              </div>
              {s.description && (
