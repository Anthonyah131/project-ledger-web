    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3">
          {report.alternativeCurrencies!.map((alt) => (
            <div
              ...
              className="rounded-xl border border-dashed border-emerald-500/20 bg-emerald-500/3 p-4"
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                />
                <AltCurrencyValue
                  label={t("reports.shared.netBalance")}
      ...
      {(report.averageIncomeAmount != null ||
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {report.averageIncomeAmount != null && (
            <SummaryCard
              label={t("reports.income.avgPerIncomeLabel")}
          ...
          {report.largestIncome != null && (
            <SummaryCard
              label={t("reports.income.largestIncome")}
      ...
      {report.sections.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t("reports.income.monthlyDetailDesc")}</CardDescription>
          </CardHeader>
              ...
              >
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    {section.topIncome && (
                      <span
                        ...
                        title={t("reports.income.topIncomeTitle", { title: section.topIncome.title })}
                      >
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap justify-end">
                    <span>{t("reports.shared.incomesCount", { count: section.sectionCount })}</span>
                ...
                {section.alternativeCurrencies && section.alternativeCurrencies.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {section.alternativeCurrencies.map((alt) => (
                      ...
                      >
                        {alt.currencyCode}: {t("reports.income.altIncome")} {formatAmount(alt.totalIncome)}
                        {` · ${t("reports.income.altNet")} ${formatAmount(alt.netBalance)}`}
                      </Badge>
                    ))}
                  </div>
                )}
                  ...
                  <>
                    ...
                    {section.sectionCount > section.incomes.length && (
                      <div className="mt-3 rounded-md border border-dashed border-muted-foreground/25 bg-muted/20 px-4 py-2.5 text-xs text-muted-foreground">
                        {t("reports.income.showingOf", { shown: section.incomes.length, total: section.sectionCount })}
                      </div>
                    )}
                  </>
                ) : (
                  <p className="text-xs text-muted-foreground">
                    {t("reports.income.noMonthDetail")}
                  </p>
                )}
              </div>
            ))}
      ...
      {hasCategoryAnalysis && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("reports.income.categoryAnalysisDesc")}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-3">
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {t("reports.shared.incomesCount", { count: cat.incomeCount })}
      ...
      {hasPaymentMethodAnalysis && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("reports.income.pmAnalysisDesc")}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-3">
              {report.paymentMethodAnalysis!.map((pm) => (
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {t("reports.shared.incomesCount", { count: pm.incomeCount })}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-24 h-2 rounded-full bg-muted overflow-hidden">
                    ...
                    <div className="flex flex-col items-end w-28">
                      ...
                      <span className="text-xs tabular-nums text-muted-foreground">
                        ~{report.currencyCode} {formatAmount(pm.averageAmount)} {t("reports.income.perIncome")}
                      </span>
                    </div>
                  </div>
                </div>
